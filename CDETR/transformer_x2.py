...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.self_attn = Mixer(dim=d_model, num_heads=nhead,qkv_bias=False, attn_drop=0., attention_head=1, pool_size=2)
    ...
    def forward_post(self,
                     ...
                     pos: Optional[Tensor] = None):
        src_mixer=src 
        print("编码层的输入", src.shape)#b c h w 16 256 8 8
        # q = k = self.with_pos_embed(src, pos)
        # src2 = self.self_attn(q, k, value=src, attn_mask=src_mask,
        #                       key_padding_mask=src_key_padding_mask)[0]
        src2 = self.self_attn(src_mixer)#torch.Size([16, 8, 8, 256])
        print("*******************",src2.shape)# torch.Size([64, 16, 256])
        src2 = src2.permute(0, 3, 1, 2)
        # src2 = src2.flatten(2).permute(2, 0, 1)
        # print("*******************最终输出",src2.shape)#([64, 16, 256])
        src3 = src.flatten(2).permute(2, 0, 1)
        src4 = src2.flatten(2).permute(2, 0, 1)
        src = src3 + self.dropout1(src4)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        print("编码层的输出",src.shape)
        return src
